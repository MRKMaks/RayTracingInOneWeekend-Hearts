    ...
    void render(const hittable& world, std::string filename) {
        ...
        for (int j = 0; j < image_height; ++j) {
            for (int i = 0; i < image_width; ++i) {
                ...
                write_color_to_buffer(image_buffer, i, j, pixel_color, samples_per_pixel);
            }
            
            #pragma omp critical
            {
                scanlines_done++;
                std::clog << "\rScanlines completed: " << scanlines_done << " / " << image_height << ' ' << std::flush;
            }
        }
        std::clog << "\nDone. Saving file...\n";
        ...
        if (ext == ".png") {
            save_success = stbi_write_png(filename.c_str(), image_width, image_height, 3, image_buffer.data(), image_width * 3);
        } else if (ext == ".bmp") {
            save_success = stbi_write_bmp(filename.c_str(), image_width, image_height, 3, image_buffer.data());
        } else if (ext == ".jpg" || ext == ".jpeg") {
            // JPEG quality scale: 1 to 100
            save_success = stbi_write_jpg(filename.c_str(), image_width, image_height, 3, image_buffer.data(), 100);
        } else {
            ...
            save_success = stbi_write_png(default_name.c_str(), image_width, image_height, 3, image_buffer.data(), image_width * 3);
        }
...
private:
    ...
    void write_color_to_buffer(std::vector<unsigned char>& buffer, int i, int j, color pixel_color, int samples_per_pixel) {
        ...
        buffer[pixel_index + 0] = static_cast<unsigned char>(256 * intensity.clamp(r));
        buffer[pixel_index + 1] = static_cast<unsigned char>(256 * intensity.clamp(g));
        buffer[pixel_index + 2] = static_cast<unsigned char>(256 * intensity.clamp(b));
    }
};
